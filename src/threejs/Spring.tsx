...
const Spring = () => {
    ...
    // 박스의 위치와 크기 설정
    const boxPosition = new THREE.Vector3(-0.5, 1, 6);
    ...
    const particles = Array.from({ length: 1000 }).map(() => ({
        ...
        velocity: new THREE.Vector3((Math.random() - 0.5) * 0.005, -0.002, (Math.random() - 0.5) * 0.005),
        ...
        if (ref.current) {
            particles.forEach((particle, i) => {
                ...
                if (particle.position.y < boxPosition.y - boxDimensions.height / 2) {
                    ...
                    particle.position.x = boxPosition.x + (Math.random() - 0.5) * boxDimensions.width;
                    particle.position.z = boxPosition.z + (Math.random() - 0.5) * boxDimensions.depth;
                }
